...
% -------------------------------------------------
ind_o = find(Mag >= Mo & Depthkm <= Depth_max & Year >= T1 & Year <= T2 & Month >= M1 & Month <= M2);
ind_m = [ind_m;ind_o];
Ind_M = ind_o;
...
set(axes1,'ClippingStyle','rectangle','Color',[0.8313725490 0.815686274509 0.78431372549],'DataAspectRatio',[1 1 1]);
% - - - - - - - -
% pinta estaciones:
geoshow('landareas.shp', 'FaceColor', [0 0 0]);hold on;  ylim([-90 85]);
xlim([-180 180]);grid
geoshow(latitud,Long_Intermagnet,'DisplayType','point','Marker', '.','MarkerEdgeColor','g','MarkerSize',17)
title('Longitude(x axes),Latitude(y axes), g=Stations');
title({['Posicion de estaciones Intermagnet, ' 'Numero de terremotos mostrados: ' num2str(length(Mag)) ]});
xlabel('Longitude ');
ylabel('Latitude');
...
for i=1:length(Mag)
    i=length(Mag)-i +1;
    geoshow(Lat(i),Lon(i),'DisplayType','point','Marker', '.','MarkerEdgeColor',[(0.18+(Mag(i)/10 - 0.09999)) color2(i) 0],'MarkerSize',exp(Mag(i)/2.45))
end
% Imprime la figura:
if imprimir==1;
...
for resumir2=1:1
    if caso==2
        ...
        [T_Dentro_Radio_indices,Distancias_respectivas] = Terremotos_Radio(Lon,Lat,ELON,ELAT,radio);
        ...
        figure1 = figure(2);
        axes1 = axes('Parent',figure1);
        set(axes1,'ClippingStyle','rectangle','Color',[0.831372549019608 0.815686274509804 0.784313725490196],'DataAspectRatio',[1 1 1]);
        geoshow('landareas.shp', 'FaceColor', [0 0 0]);hold on;
        xlim([(x(50)-1) (x(100)+1)]);ylim([y(75) y(25)]);
        grid
        geoshow(latitud,Long_Intermagnet,'DisplayType','point','Marker', 'x','LineWidth',10,'MarkerEdgeColor',[1.0000    0.8431         0],'MarkerSize',24)
        geoshow(latitud(Numero_Estacion), Long_Intermagnet(Numero_Estacion),'DisplayType','point','Marker', '.','LineWidth',3,'MarkerEdgeColor','g','MarkerSize',12)
        text(Long_Intermagnet,latitud+1.15,IAGA,'Color',[ 0 0.4980 0],'FontSize',13,'HorizontalAlignment','Center','Backgroundcolor',[0.9412 0.9412 0.9412])
        title(['Geographical position and events, central Station:' char(IAGA(Numero_Estacion)) ', Date:'  num2str(Time1) ' to 'num2str(Time2)])
        %- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for i=1:length(MagS)
            i=length(MagS)-i +1;
            geoshow(LatS(i),LonS(i),'DisplayType','point','Marker', '.','MarkerEdgeColor',[(0.29+(MagS(i)/10 - 0.09)) color2(i) 0],'MarkerSize',(MagS(i)^2.2))
            geoshow(LatS(i),LonS(i),'DisplayType','point','Marker', '.','MarkerEdgeColor',[(0.29+(MagS(i)/10 - 0.09)) 0.5 color2(i)],'MarkerSize',(MagS(i)^1.8))
        end
        plot(x,y,'--','LineWidth',3)
        % TABLA DISTANCIAS FINAL:
        if length(Distancias_respectivas)>=1 %Evita un error
            T=table(MagS,round(Distancias_respectivas),LatS,LonS,DayS,MonthS,YearS,TimeUTCS,TimestampS,DepthkmS,IRISIDS,'VariableNames',{'Magnitud' 'DistanciasAEstacionKm' 'Latitud' 'Longitud' 'Dia' 'Mes' 'Year' 'TimeUTC' ...
                'Timestamp' 'DepthkmS' 'IRISIDS'});
        else
            T=table(num2str(MagS),round(Distancias_respectivas),num2str(LatS),num2str(LonS),DayS,MonthS,YearS,TimeUTCS,...
                num2str(TimestampS),IRISIDS,DepthkmS,'VariableNames',{'Magnitud' 'DistanciasAEstacionKm' 'Latitud' 'Longitud' ...
...
for resumir1=1:1
    if caso==1
        ...
        ELAT1=latitud;
        [E_Dentro_Radio_indices,Distancias_respectivasE] =Estaciones_Radio(LatS1,LonS1,ELON1,ELAT1,radio);
        % SALIDA:
        % TABLA DISTANCIAS FINAL:
        display({'Magnitud:' num2str(Mag(Numero_Terremoto))});
        if length(Distancias_respectivasE)>=1
            T2=table((IAGA(E_Dentro_Radio_indices),round(Distancias_respectivasE),Long_Intermagnet(E_Dentro_Radio_indices),...
                ELAT1(E_Dentro_Radio_indices),Country(E_Dentro_Radio_indices), 'VariableNames',{'IAGA' 'Distancias_km' 'Longitud' 'Latitud' 'Country'});
        else
            T2=table(IAGA(E_Dentro_Radio_indices),round(Distancias_respectivasE),num2str(Long_Intermagnet(E_Dentro_Radio_indices)),...
                num2str(ELAT1(E_Dentro_Radio_indices)),Country(E_Dentro_Radio_indices),'VariableNames',{'IAGA' 'Distancias_km' 'Longitud' 'Latitud' 'Country'});
        end
...
% NO LO RECOMIENDO:
for resumir3=1:1
    if caso==3 %Visualiza mapa y pinta las estaciones con su nombre
        close (figure(3))
        figure(3)
        ax = worldmap('World');
...
if caso == 4;
    ...
    for std_f =1:length(Label_Station)
        ...
        [T_Dentro_Radio_indices,Distancias_respectivas] = Terremotos_Radio(Lon,Lat,ELON(std_f),ELAT(std_f),r_todo(std_f));
    ...
    figure1 = figure(2);
    axes1 = axes('Parent',figure1);
    set(axes1,'ClippingStyle','rectangle','Color',[0.831372549 0.815686274 0.78431372],'DataAspectRatio',[1 1 1]);
    geoshow('landareas.shp', 'FaceColor', [0 0 0]);hold on;
    xlim([(Min_x-2) (Max_x+2)]);
    ylim([Min_y-2 Max_y+2]);
    grid,geoshow(latitud(Ind_station),Long_Intermagnet(Ind_station),'DisplayType','point','Marker', 'x','LineWidth',10,'MarkerEdgeColor'...
        ,[1.0000 0.8431 0],'MarkerSize',24)
    geoshow(latitud(Ind_station),Long_Intermagnet(Ind_station),'DisplayType','point','Marker', '.','LineWidth',3,'MarkerEdgeColor',...
        'g','MarkerSize',12)
    text(Long_Intermagnet(Ind_station),latitud(Ind_station)+1.15,IAGA(Ind_station),'Color',[ 0 0.4980 0],'FontSize',9,'HorizontalAlignment','Center','Backgroundcolor',[0.9412 0.9412 0.941])
    ...
    for std_f= 1: length(Label_Station)
        ...
        if  length(EventF.Stat(std_f).Dist)>=1
            switch std_f
                case 1,
                    T1 = table(MagS,round(EventF.Stat(std_f).Dist),LatS,LonS,DayS,MonthS,YearS,TimeUTCS,TimestampS,DepthkmS,...
                        IRISIDS,'VariableNames',{'Magnitud' 'DistanciasAEstacionKm' 'Latitud' 'Longitud' 'Dia''Mes' 'Year' 'TimeUTC' 'Timestamp' 'DepthkmS''IRISIDS'});
                case  2,
                    T2 = table(MagS,round(EventF.Stat(std_f).Dist),LatS,LonS,DayS,MonthS,YearS,TimeUTCS,TimestampS,DepthkmS,...
                        IRISIDS,'VariableNames',{'Magnitud' 'DistanciasAEstacionKm' 'Latitud' 'Longitud' 'Dia''Mes' 'Year' 'TimeUTC' 'Timestamp' 'DepthkmS''IRISIDS'});
                case  3,
                    T3 = table(MagS,round(EventF.Stat(std_f).Dist),LatS,LonS,DayS,MonthS,YearS,TimeUTCS,TimestampS,DepthkmS,...
                        IRISIDS,'VariableNames',{'Magnitud' 'DistanciasAEstacionKm' 'Latitud' 'Longitud' 'Dia''Mes' 'Year' 'TimeUTC' 'Timestamp' 'DepthkmS''IRISIDS'});
        ...
        for i=1:length(MagS)
            hold on
            i=length(MagS)-i +1;
            geoshow(LatS(i),LonS(i),'DisplayType','point','Marker', '.','MarkerEdgeColor',[(0.18+(MagS(i)/10 - 0.09999)) color2(i) 0],'MarkerSize',(MagS(i)^1.7))
        end
    ...
    plot([P1(1),P2(1),P3(1),P1(1)],[P1(2),P2(2),P3(2),P1(2)],'B-','LineWidth',1.5)
    ...
    text((P1(1)+P2(1))/2,(P1(2)+P2(2))/2,{[num2str(dist_std(1)) ' km']},'Color','b','FontSize',7.5,'HorizontalAlignment','Center','Backgroundcolor',[0.9412    0.9412    0.9412])
    text((P1(1)+P3(1))/2,(P1(2)+P3(2))/2,{[num2str(dist_std(2)) ' km']},'Color','b','FontSize',7.5,'HorizontalAlignment','Center','Backgroundcolor',[0.9412    0.9412    0.9412])
    text((P2(1)+P3(1))/2,(P2(2)+P3(2))/2,{[num2str(dist_std(3)) ' km']},'Color','b','FontSize',7.5,'HorizontalAlignment','Center','Backgroundcolor',[0.9412    0.9412    0.9412])
    %   legend:
    legend_text={['Station: ' char(Label_Station(1)) ' Radius: ' num2str(r1)' km';'Station: ' char(Label_Station(2)) ' Radius: ' num2str(r2) ...
        ' km';'Station: ' char(Label_Station(3)) ' Radius: ' num2str(r3) ' km']};
    text(Max_x+6.5,Max_y,legend_text,'Color','b','FontSize',8,'HorizontalAlignment','Center','Backgroundcolor',[0.9412    0.9412    0.9412])
    % titulo:
    title(['Geographical position, Stations: ' char(Label_Station(1)) ', ' char(Label_Station(2)) ', ' char(Label_Station(3)) ', Date: ' ...
    ...
    T = [];
    T = [T1; T2; T3];
    T.DistanciasAEstacionKm=[T.DistanciasAEstacionKm(:) T.DistanciasAEstacionKm(:) T.DistanciasAEstacionKm(:)];
    ...
    for std = 1:length(Label_Station)
        ...
        for indtablat=1:length(T.Latitud)
            ...
            d_st_ev_ = round(sqrt(dx_Terremotoalternativo.^2 + dy_Terremotoalternativo.^2));
